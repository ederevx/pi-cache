...
5. **Prefix continuance** — a fourth Anthropic cache_control marker is
   ...
   and a real turn exists (`PI_CACHE_FORCE_WARM`) — the warming
   schedule follows the tier the per-request rewrite actually put on
   the wire, idle fires defer past an in-flight refresh (margin plus
   round-trip grace), and confirmed refreshes land in the ledger as
   `warm`-flagged rows.
