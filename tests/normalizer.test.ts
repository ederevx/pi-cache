 ...
 * normalize() must dedup tools, re-pin a trailing Anthropic cache_control
 * marker to the new last tool, leave a mid-array explicit breakpoint
 * untouched, and count prefix-head churn.
 */
...
test("normalizer: dedups exact-duplicate tools", () => {
  const n = new PrefixNormalizer({ dedupTools: true });
  const payload = {
    model: "m",
    messages: [],
...
test("normalizer: re-pins a trailing cache_control marker after dedup", () => {
  const n = new PrefixNormalizer({ dedupTools: true });
  const payload = {
    model: "m",
    messages: [],
    // Duplicates first, marked tool last: the marker stays pinned to the
    // final tool through the dedup.
    tools: [
      makeTool("a"),
      makeTool("a"),
  ...
  assertDeepEq(tools.map((t) => t.name), ["a", "c"], "deduped");
...
test("normalizer: unchanged input returns the same reference", () => {
  ...
  assertEq(n.normalize(payload), payload, "already deduped stays identical");
});
...
test("normalizer: churn counts head changes, first observation is baseline", () => {
  const n = new PrefixNormalizer({ dedupTools: false });
