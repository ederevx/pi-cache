 ...
 * routing; the view only renders them. Pin the row order, which rows are
 * editable, and that the TUI path builds the custom view while other modes
 * fall back to the stderr listing.
 */
...
import type { PiCacheOptions } from "../src/constants.ts";
...
const OPTS = {
  ...
  advisory: false,
  autoCompact: true,
} as unknown as PiCacheOptions;

const LIVE: LiveSettings = { fastCompaction: true, fastBranchSummary: false };

test("settings: rows keep the option order and mark only the live switches editable", () => {
  const rows = new SettingsPresenter().rows(OPTS, LIVE);
  ...
  assertEq(
    rows.filter((r) => r.editable).map((r) => r.id).join(","),
    "fastCompaction,fastBranchSummary",
  );
  assertEq(rows[1].value, "off", "env option reflected");
...
test("settings: tui mode renders the two-column view through the custom UI", async () => {
  ...
  await new SettingsPresenter().present(OPTS, LIVE, ui as never, "tui", () => {});
